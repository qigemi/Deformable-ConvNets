...
inc_sec= {  2: 16, \
            ...
            4: 24, \
            5: 128 }
def get_dpn92_conv4_merge(data):
    conv4_x_x = get_dpn92_conv4(data)
    conv4_x_x  = mx.symbol.Concat(*[conv4_x_x[0], conv4_x_x[1]],  name='conv4_x_x_cat-final')
    conv4_x_x = BN_AC(conv4_x_x, name='conv4_x_x__relu-sp')
    return conv4_x_x

def get_dpn92_conv4(data):
    ...
    return conv4_x_x
def get_dpn92_conv5(conv_feat):
    # conv5
    bw = 2048
    inc= inc_sec[5]
    R  = (k_R*bw)/256
    conv5_x_x  = DualPathFactory(     conv_feat,   R,   R,   bw,  'conv5_x__1',           inc,   G,  'down' ,True )
...
def get_before_pool():
    ...
    conv4_x_x = get_dpn92_conv4(img_data=data)
    # conv5
    bw = 2048
    inc= inc_sec[5]
    R  = (k_R*bw)/256
