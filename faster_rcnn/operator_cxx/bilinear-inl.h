...
*/
#ifndef MXNET_OPERATOR_BILINEAR_INL_H_
#define MXNET_OPERATOR_BILINEAR_INL_H_
...
namespace mxnet {
  namespace op {
    ...
    namespace conv {
      enum BilinearOpInputs { kData,kTarget};
      enum BilinearOpOutputs { kOut };
      enum BilinearOpResource { kTempSpace };
    }

    struct BilinearParam : public dmlc::Parameter<BilinearParam> {
      TShape data_shape;
      TShape target_shape;
      uint64_t workspace;
      dmlc::optional<int> layout;
      DMLC_DECLARE_PARAMETER(DeformableConvolutionParam) {
        DMLC_DECLARE_FIELD(target_shape).set_default(TShape()).describe("resize output size");
    ...
    class BilinearOp : public Operator {
    public:
      explicit BilinearOp(BilinearParam p) {
        this->param_ = p;
