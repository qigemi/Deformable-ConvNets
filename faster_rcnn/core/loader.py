...
class AnchorLoader(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, cfg, batch_size=1,class_num = 21, shuffle=False, ctx=None, work_load_list=None,
                 feat_stride=16, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2), allowed_border=0,
                 aspect_grouping=False,multi_label = False):
        """
        This Iter will provide roi data to Fast R-CNN network
        ...
        self.label_name = ['label', 'bbox_target', 'bbox_weight']
        if multi_label:
            self.label_name.append('multi_label')
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        label = assign_anchor(feat_shape[0], np.zeros((0, 5)), im_info, self.cfg,self.class_num,
                              self.feat_stride, self.anchor_scales, self.anchor_ratios, self.allowed_border,self.multi_label)
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            label = assign_anchor(feat_shape, label['gt_boxes'], data['im_info'], self.cfg,self.class_num,
                                  self.feat_stride, self.anchor_scales,
                                  self.anchor_ratios, self.allowed_border,self.multi_label)
    ...
    def parfetch(self, iroidb):
        ...
        # assign anchor for label
        label = assign_anchor(feat_shape=feat_shape, gt_boxes=label['gt_boxes'], im_info=data['im_info'],cfg= self.cfg,
                              class_num=self.class_num,feat_stride=self.feat_stride,scales= self.anchor_scales,
                              ratios=self.anchor_ratios, allowed_border=self.allowed_border,multi_label = self.multi_label)
        return {'data': data, 'label': label}
